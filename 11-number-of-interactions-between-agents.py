...
# Créer 2 agents spécialisés avec instructions
stock_agent = Agent(
    name="StockAgent",
    model=OpenAIChat(id="gpt-4o-mini", api_key=OPENAI_API_KEY),
    tools=[YFinanceTools()],
    instructions="""Tu es un agent spécialisé dans les données financières.
    Ton rôle est de récupérer les prix d'actions en temps réel.
    Utilise toujours l'outil get_current_stock_price pour les requêtes boursières.
    Fournis des réponses claires et précises sur les prix des actions."""
)
...
calc_agent = Agent(
    ...
    instructions="""Tu es un agent spécialisé dans les calculs mathématiques.
    Ton rôle est d'effectuer des opérations arithmétiques précises.
    Utilise les outils de calcul (add, subtract, multiply, divide) pour toutes les opérations.
    Fournis des résultats numériques exacts et détaillés."""
)
