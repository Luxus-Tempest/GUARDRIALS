...
agent = Agent(
    ...
    tools=[YFinanceTools()],
    markdown=True,
    instructions="""You are a financial tools selection agent.
    Your role is to choose the right tools for each task.
    Use get_current_stock_price for stock market queries.
    Analyze each request to select the appropriate tool."""
)
...
evaluation_tasks = [
    {
        ...
        "input": "Compare the stock prices of Tesla and Apple",
        "expected_output": "The agent should use the get_current_stock_price tool twice - once for TSLA and once for AAPL",
        "additional_guidelines": "The agent must recognize this requires multiple tool calls to get both stock prices for comparison."
    },
    {
        "name": "No Tool Selection - Out of Scope",
        "input": "Get me the weather forecast for Paris",
        "expected_output": "The agent should NOT use any tools and explain that it only handles stock price queries",
        "additional_guidelines": "The agent must recognize this is outside its scope and not attempt to use any tools."
    }
]
...
if accuracy_metrics:
    ...
    correct_tool_tasks = [m for m in accuracy_metrics if "Correct Tool" in m['task_name']]
    no_tool_tasks = [m for m in accuracy_metrics if "No Tool" in m['task_name']]
    error_handling_tasks = [m for m in accuracy_metrics if "Invalid" in m['task_name']]
    
    complex_tasks = [m for m in accuracy_metrics if "Multiple" in m['task_name'] or "Complex" in m['task_name']]
