...
agent = Agent(
    ...
    tools=[YFinanceTools()],
    markdown=True,
    instructions="""You are a financial formatting agent.
    Your role is to provide responses in specific formats.
    Use get_current_stock_price for stock market data.
    Strictly respect the requested output formats."""
)

# Définir les tâches d'évaluation avec AccuracyEval
evaluation_tasks = [
    {
        "name": "AAPL Stock Price Format Accuracy",
        "input": "What is the current stock price of AAPL? Please format your response as: SYMBOL: [symbol] | VALUE: $[amount]",
        "expected_output": "SYMBOL: AAPL | VALUE: $[amount]",
        "additional_guidelines": "The response should follow the exact format specified with SYMBOL: and VALUE: labels and include the dollar sign."
    },
   
    {
        "name": "Error Format Handling",
        "input": "What is the current stock price of FAKE_STOCK? Format error as: FAILED: [message]",
        "expected_output": "FAILED: [message]",
        "additional_guidelines": "The agent should handle invalid stock symbols gracefully and format the error message as specified."
    },
    {
        "name": "JSON Format Accuracy",
        "input": "Get MSFT stock price. Format as JSON: {\"stock\": \"MSFT\", \"price\": \"$[price]\"}",
        "expected_output": "{\"stock\": \"MSFT\", \"price\": \"$[price]\"}",
        "additional_guidelines": "The response should be formatted as valid JSON with stock symbol and price fields."
    }
]
...
for i, task_data in enumerate(evaluation_tasks, 1):
    ...
    evaluation = AccuracyEval(
        name=task_data['name'],
        model=OpenAIChat(id="gpt-4o-mini", api_key=OPENAI_API_KEY),  # Modèle pour l'évaluation
